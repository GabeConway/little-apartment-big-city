...
// scale with accuracy.
//
// AUDIO: the song IS the minigame. A dedicated 60s cut — "Midnight Neon"
// (intro → chorus → verse → chorus, faded at the cadence) — plays from 0:00 while
...
const KARAOKE_CHART: number[] = [
  ...
  29.33, 29.80, 30.26, 31.19, 32.12, 33.98, 34.91, 36.77, 37.23, 37.70, 38.63, 39.56,
  40.95, 41.42, 41.88, 42.81, 44.21, 46.07, 47.00, 47.93, 48.86, 49.79, 50.72, 51.65,
  52.58, 53.51, 54.44, 55.37, 56.30, 57.23, 59.09, 59.55, 60.02,
];
...
// Chart times are mp3-relative; game time adds the count-in in front.
const makeKaraokeGame = (): KaraokeGame => ({
  t: 0, audioOn: false,
  notes: KARAOKE_CHART.map(t => ({ t: t + KARAOKE_COUNTIN_SEC, judged: 'none' as const })),
...
const LittleApartmentGame: React.FC = () => {
  ...
  const startKaraoke = () => {
    ...
    karaokeRef.current = makeKaraokeGame();
  };
