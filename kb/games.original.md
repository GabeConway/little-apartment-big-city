...
**Naganoshi** — Japanese pixel font (OFL 1.1, GGBotNet) — self-hosted in `public/fonts/` (`@font-face` in `src/index.css`; `font-jp` Tailwind family; license bundled as `public/fonts/Naganoshi-License.txt`). Source archive (not bundled): `assets/fonts/Naganoshi_Font_0_4.zip`. The canvas store signs render **per character**: kana/kanji in Naganoshi, Latin/digits/punctuation in bold monospace (Naganoshi has no Latin glyphs, so a whole-string switch would tofu the English parts). See `measureRun`/`drawRun`/`charFont` in the sign-drawing block of `LittleApartmentGame.tsx`. Canvas text doesn't auto-fetch a webfont — `document.fonts.load("16px 'Naganoshi'")` is kicked in the play-loop effect.

## localStorage
`lab-save` (v2), `lab-music-muted`, `lab-scale` (`'auto'` or a forced integer canvas scale 1–8; title screen **DISPLAY** panel).

## Display / scaling
Title screen **DISPLAY** button → panel of scale options (AUTO / 1×–6×), persisted as `lab-scale`. The `fit()` effect: **AUTO** = biggest integer multiple of 384×224 that fits the frame (the layout box). **N×** = forced scale, clamped so it can't exceed the screen — in the **filled** layout (fullscreen/native) the frame *is* the viewport so it's the cap; in **windowed** web the box is capped at 1000px, so a forced scale is clamped to the *viewport* (`window.innerWidth/innerHeight`) instead and `boxW` grows the box past 1000 to fit (never shrinks below 1000 — that would crowd the HUD). Without this, every scale above 2× clamped to the box and looked identical. (No separate "resolution" axis — logical viewport fixed at 384×224 by map design; output res = scale × that.)

## Verifying changes
`npx tsc --noEmit` + `npm test` + `npm run build`, then **`npm run playtest`** (Playwright harness — seed a save, drive input, read `window.__lab.snapshot()`, screenshot; see [kb/playtesting.md](playtesting.md)). Tricks: inject a crafted `lab-save` (+`v:2`, include relevant `storySeen` ids or a pending story letter eats input — the harness auto-dismisses it unless `--keep-overlay`) → reload → CONTINUE teleports anywhere; `--dpr 2` for Retina scaling checks; `--save '{scene,px,py}'` teleports via `begin()`. Map sanity (row widths, warp/ore/spawn walkability) via regex over `maps.ts`.
