...
| `state.ts` | `lab-save` v2 (versioned; v1 saves auto-place at legacy slots on load — keep `v` check), clock (`timeMin`, wake 7:00, collapse 26:00), `nightT` tint curve, placement helpers, pawn/sketchy daily seeds, `oreNodesFor`, `shrineLuck`, `unlockGameAch` |
