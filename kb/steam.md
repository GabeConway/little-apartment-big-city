...
|---|---|---|
| **Windows** | portable `little-apartment.exe` + resources (already built by CI) | Ship first — basically depot-ready. **WebView2 is the one catch**: not in Steam's common-redistributables list, so add an InstallScript running the Evergreen bootstrapper on first install (Win11 preinstalled; Win10 needs it). This is the well-known Tauri-on-Steam friction point. |
