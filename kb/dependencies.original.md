...
- `playwright` 1 — **dev-only** playtest harness (`scripts/playtest.mjs`; never imported by `src/`, so it doesn't enter the bundle). See [playtesting.md](playtesting.md). Chromium browser via `npx playwright install chromium`.
